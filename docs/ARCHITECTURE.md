# @request-suite/request-api-client Architecture
...
> Contracts (OpenAPI spec, metadata, webhook fixtures) live in `@request-suite/request-client-contracts` and are consumed via package imports during build/test steps.
```
...
| Subpath | Purpose |
| --- | --- |
| `@request-suite/request-api-client/requests` | Requests & invoices facade helpers |
| `@request-suite/request-api-client/payouts` | Payout creation and execution helpers |
| `@request-suite/request-api-client/payer` | Compliance and payer onboarding helpers |
| `@request-suite/request-api-client/currencies` | Currency listing and conversion routes |
| `@request-suite/request-api-client/client-ids` | Client ID lifecycle management |
| `@request-suite/request-api-client/payments` | Payment search facade with pagination helpers |
| `@request-suite/request-api-client/pay` | Legacy pay execution facade (mirrors `/v1/pay`) and exposes `.legacy` |
| `@request-suite/request-api-client/v1/requests` | Legacy request endpoints and status helpers |
| `@request-suite/request-api-client/v1/payer` | Legacy payer/compliance endpoints |
| `@request-suite/request-api-client/v2/payer` | Explicit v2 payer barrel for tree-shaking |
| `@request-suite/request-api-client/v1/currencies` | Legacy currency list and conversion routes |
| `@request-suite/request-api-client/v1/pay` | Versioned legacy pay facade factory |
...
- **Environment variables** - `createRequestClientFromEnv()` searches for canonical variables first:
  - `REQUEST_API_URL`
  - `REQUEST_API_KEY`
  - `REQUEST_CLIENT_ID`
  If any are missing, it falls back to legacy names (`REQUEST_SDK_BASE_URL`, `REQUEST_SDK_API_KEY`, `REQUEST_SDK_CLIENT_ID`, `REQUEST_SDK_LOG_LEVEL`) to ease migration. Document both sets in README/testing guides and plan to deprecate the legacy prefixes in a future major release.
...
- **Legacy barrels** - When backwards compatibility is required, publish explicit subpath barrels such as `@request-suite/request-api-client/v1/requests`. The barrel re-exports the same facade factory wired to v1 paths. Default exports must remain v2-only so new integrations land on the latest API automatically.
...
- **Spec alignment** - Ensure `@request-suite/request-client-contracts/specs/openapi/request-network-openapi.json` and generated types match the version each barrel targets. Regenerate and diff specs when adopting new versions.
...
- A parity guard (`tests/webhooks/event-parity.test.ts`) keeps `WEBHOOK_EVENT_NAMES` aligned with `@request-suite/request-client-contracts/specs/webhooks/request-network-webhooks.json`. Update the spec first when introducing new events.
